...
//		}
		...
		if (GUILayout.Button ("GetServerCSVHash", GUILayout.Width (150))) {
			serverHash = FileManager.GetFileHash (PathConstant.CLIENT_SERVER_VERSION_CSV);
		}
		EditorGUILayout.EndHorizontal ();
	}
	...
	static void GetHash ()
	{
		...
		for (int i = 0; i < allAssetBundleEntitys.Count; i++) {
			...
			allAssetBundleEntitys [i].abHash = FileManager.GetFileHash (PathConstant.CLIENT_ASSETBUNDLES_PATH + allAssetBundleEntitys [i].abName);
			if (allAssetBundleEntitys [i].abHash != null && allAssetBundleEntitys [i].abHash.Trim () != "") {
				allAssetBundleEntitys [i].realLength = new FileInfo (PathConstant.CLIENT_ASSETBUNDLES_PATH + allAssetBundleEntitys [i].abName).Length;
